...
export class ReviewEventComponent implements OnInit, OnDestroy {
  ...
  updateReviewData() {
    ...
    if (this.errorMessages.length > 0) {
      this.reviewStatusAnalysisOK.emit(false);
    } else {
      this.reviewStatusAnalysisOK.emit(true);
    }
  ...
  */
  checkCircleProperties() {
    ...
    this.circles.forEach( circle => {
      ...
      this.eventActivatedDots.forEach( (event: CircleActivatedDots) => {
        if (event.index === bestFitIndex) {
          ...
          if (Math.abs(event.radius - circle.r)/event.radius > 0.01) {
            console.log(event.radius, typeof event.radius);
            console.log(circle.r, typeof circle.r);
            radiusChecks.push({
              circleRadius: circle.r,
              message: `The fit radius for the circle with r = ${(circle.r).toFixed(4)} cm differs by more than 1% from
              the correct value.  You should check that the fit includes all the points for the particle's path and 
              does not accidentally include points from other circles.\n\n  To check the points that are included in the fit, 
              go back to the previous step and hover over the 'Edit circle properties' box for this circle.  To fix the fit, start
    ...
    if (this.errorMessages.length === 0) {
      ...
      radiusChecks.forEach( check => this.errorMessages.push(check.message));
    }
